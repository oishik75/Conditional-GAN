...
def gradient_penalty(critic, real, generated, labels, device):
    ...
    mixed_critic_scores = critic(interpolated_images, labels)
...
def train(generator, critic, dataloader, device, args):
    ...
    for epoch in tqdm(range(args.n_epochs)):
        for batch_idx, (real, labels) in tqdm(enumerate(dataloader)):
            ...
            for _ in range(args.critic_iterations):
                critic_real_output = critic(real, labels).reshape(-1) # Shape: batch_size 
                noise = torch.randn((BATCH_SIZE, args.z_dim, 1, 1)).to(device)
                generated = generator(noise, labels)
                critic_generated_output = critic(generated, labels).reshape(-1)
                ...
                if args.use_gradient_penalty:
                    gp = gradient_penalty(critic, real, generated, labels, device)
            ...
            noise = torch.randn((BATCH_SIZE, args.z_dim, 1, 1)).to(device)
            critic_generated_output = critic(generator(noise, labels), labels).reshape(-1)
...
def main():
    ...
    generator = Generator(args.z_dim, args.image_channels, args.features_g, args.n_classes, args.gen_embed_dim).to(device)
    initialize_weights(generator)
    critic = Critic(args.image_channels, args.features_d, args.n_classes, args.image_size).to(device)
